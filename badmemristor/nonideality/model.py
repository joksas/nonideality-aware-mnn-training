...
def lognormal(G, lognormal_G, lognormal_mean, lognormal_sigma, lognormal_rate):
    ...
    This type of modelling was inspired largely by [1]. The main difference
    is that the lognormal parameters are interpolated using linear
    interpolation between neighbouring points, instead of global linear fit.
    ...
    rates = np.interp(
        G_disturbed, lognormal_G, lognormal_rate)
    ...
    occurrences = np.where(
        np.random.random_sample(G.shape) <= rates, 1, 0)
...
def weibull(G, weibull_G, weibull_shape, weibull_scale):
    ...
    linear interpolation between neighbouring points, instead of global
    linear fit.
    ...
    weibull_G : list of float
        A set of conductances at which shape and scale parameters were estimated.
    weibull_shape : list of float
        A set of Weibull shape parameters corresponding to states in `weibull_G`.
    weibull_scale : list of float
        A set of Weibull scale parameters corresponding to states in `weibull_G`.
