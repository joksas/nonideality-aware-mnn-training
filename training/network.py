    ...
    if iterator.use_generator:
        ...
        if keras.__version__[0]=="2":
            history=model.fit_generator(
                    ...
                    nb_epoch=iterator.training.num_epochs,
                    validation_split=0.1,
                    verbose=2,
                    callbacks=[cback])
        if keras.__version__[0]=="1":
            history=model.fit_generator(
                    datagen.flow(iterator.x_train, iterator.y_train,
                        batch_size=iterator.training.batch_size),
                    samples_per_epoch=iterator.x_train.shape[0],
                    nb_epoch=iterator.training.num_epochs,
                    verbose=2,
                    validation_split=0.1,
                    callbacks=[cback])
